...
choreo :: (ArrowIO ar, Strong ar) => Choreo ar () (Integer @ "alice", Integer @ "alice")
choreo =
  Comp (LmapBridge (const $ wrap ()) IdBridge)
    (Local alice $ arr $ const (5 :: Integer)) $
  Comp IdBridge
    (Comm alice bob) $
  Comp (LRouterBridge DupRoute $
        FirstBridge IdBridge)
    (Local bob $ arr $ const (6 :: Integer)) $
  Comp
    (LRouterBridge
      (CompRoute DupRoute $
       CompRoute (AppFirst FstRoute) $
       AppSecond $
       FunRoute $ uncurry pack) $
     SecondBridge IdBridge)
    (Local bob $ arr $ uncurry (+)) $
  Comp (FirstBridge IdBridge)
    (Comm bob alice) $
  Comp (SecondBridge IdBridge)
    (Comm bob alice) $
  Comp (LRouterBridge
         (CompRoute DupRoute $
          AppSecond $
...
-- optimize (Comp _ (Comm (l :: Proxy l) (l' :: Proxy l')) (Comp _ (Comm (m :: Proxy m) (m' :: Proxy m')) _)) =
--   _
